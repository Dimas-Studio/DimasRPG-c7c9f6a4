...
// Класс для работы с файлом конфига урона существ
public class DimasRPGMobDamageCommonConfig {
    ...
    public static void initConfig(String mod_id, String config_folder) {
        ...
        ConfigMobDamageValues.setDefaultConfigValues();
        ...
        File file = folderPath.resolve(mod_id + "-mob-damage-common.json").toFile();
        if(!file.exists()) {
            // Конфиг файл не найден, создаём новый с значениями по умолчанию
            DimasRPG.LOGGER.info("Could not find mob damage config, generating new default config.");
            saveConfig(file);
        }
        else {
    ...
    private static void readConfig(File file) {
        try {
            ...
            if (!validateConfig(json)) {
                DimasRPG.LOGGER.error("Mod damage config file have wrong syntax, use default config.");
            } else {
                // Очитска значений по умолчанию и последующее заполнение параметрами из файла
                ConfigMobDamageValues.clearDefaultConfigValues();
                ...
                for (Map.Entry<String, JsonElement> entry : json.entrySet()) {
                    ...
                    ConfigMobDamageValues.put(name, type, value);
                }
            }
    ...
    private static void saveConfig(File file) {
        ...
        Object[] names = ConfigMobDamageValues.getKeys().toArray();
        Arrays.sort(names);
        ...
        for(Object name : names) {
            if(((String) name).matches("\\w+:\\w+")) {    // Провкрка на: "minecraft:creeper"
                ...
                String type = ConfigMobDamageValues.getType((String) name);
                Float value = ConfigMobDamageValues.getValue((String) name);
